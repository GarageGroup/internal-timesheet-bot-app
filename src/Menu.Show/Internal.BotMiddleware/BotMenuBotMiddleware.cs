...
internal static partial class BotMenuBotMiddleware
{
    private static async ValueTask<Unit> StartWithCommandAsync(this IBotContext botContext, string command, CancellationToken cancellationToken)
    {
    ...
    private static async ValueTask<Unit> ReplaceMenuActivityAsync(this IBotContext botContext, IActivity menuActivity, CancellationToken cancellationToken)
    {
        var menuResourceAccessor = botContext.GetMenuResourceAccessor();
